...
export default function Form({ customers = [], template }) {
    ...
    const submit = (event) => {
        ...
        const headerRow = Number(data.header_row);
        const dataStartRow = Number(data.data_start_row);

        if (dataStartRow <= headerRow) {
            setError("data_start_row", "Data Start Row must be greater than Header Row.");
            return;
        }
    ...
    const readExcelPreview = async (file = sampleFile, sheetIndex = data.sheet_index || 0) => {
        if (!file) {
            setPreviewError("Please choose a sample SR file first.");
            return;
        }
        ...
        try {
            ...
            if (!result.success) {
                setPreviewError(result.error || "Failed to read Excel file.");
                return;
            }
    ...
    const validateTemplatePreview = async () => {
        if (!sampleFile) {
            setPreviewError("Upload a sample SR file first to validate template.");
            return;
        }
        ...
        try {
            ...
            if (!result.success) {
                setPreviewError(result.error || "Template is unable to parse the sample file.");
                return;
            }
                ...
                <form onSubmit={submit} className="rounded-2xl border border-gray-200 bg-white shadow-sm">
                    <div className="border-b border-gray-100 p-6">
                        ...
                        <p className="mt-1 text-sm text-gray-500">
                            The template is created once per customer from a sample Excel file, and then used automatically for subsequent SR uploads.
                        </p>
                    </div>
                    ...
                    <div className="border-b border-gray-100 p-6">
                        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_220px]">
                            <Field
                                ...
                                hint="Upload a sample file first. Once the file is read, select the correct sheet before validating the mapping."
                            >
                                <input
                                    type="file"
                                ...
                                <div className="flex flex-col gap-3 border-b border-gray-200 bg-gray-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500">
                                            Select the sheet containing the SR data. Use column letters and row numbers from this table to fill the template.
                                        </p>
                                    </div>
                                    <select
                    ...
                    <div className="grid gap-6 p-6 lg:grid-cols-2">
                        ...
                        <Field label="Orientation" error={errors.orientation}>
                            <select className={inputClass} value={data.orientation} onChange={(e) => setData("orientation", e.target.value)}>
                                <option value="vertical">Vertical: one row per order</option>
                                <option value="horizontal">Horizontal: qty spread across date columns</option>
                            </select>
                        </Field>

                        <Field label="Sheet Index" hint="Leave empty if this template can be applied to any worksheet." error={errors.sheet_index}>
                        ...
                        {columnFields.map(([key, label]) => {
                            if (data.orientation === "vertical" && ["qty_start_column", "qty_end_column"].includes(key)) {
                                return null;
                            }
                            if (data.orientation === "horizontal" && ["qty_column", "etd_column"].includes(key)) {
                                return null;
                            }
                            return (
                        ...
                        {data.orientation === "horizontal" && (
                            <Field label="Date Header Row" hint="Required for horizontal. The row containing dates above the qty columns." error={errors.date_header_row}>
                        ...
                        <Field label="Date Format" hint="Optional, e.g. d/m/Y. Leave empty if Excel date format is normal." error={errors.date_format}>
                            <input className={inputClass} value={data.date_format} onChange={(e) => setData("date_format", e.target.value)} />
                        </Field>

                        <Field label="Skip Keywords" hint="Separate with commas." error={errors.skip_keywords}>
                    ...
                    <div className="border-t border-gray-100 p-6">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p className="text-xs text-gray-500">Verify whether this configuration yields the correct SR data before saving.</p>
                            </div>
                            <button
                                type="button"
